...
# -*- coding: utf-8 -*-
"""
University Data Warehouse Dashboard
...
view = st.sidebar.radio(
    ...
    ["👨‍🏫 Dashboard Professeur", "👨‍🎓 Dashboard Étudiant"],
)
st.sidebar.divider()
...
if view == "👨‍🏫 Dashboard Professeur":
    st.title("👨‍🏫 Tableau de Bord Professeur")

    # ── Load dimension lists ──────────────────
    modules_df = query(engine, "SELECT code_module, intitule, semestre, annee_etude FROM DIM_MODULE ORDER BY semestre, intitule")
    temps_df   = query(engine, "SELECT id_temps, annee_scolaire, semestre FROM DIM_TEMPS ORDER BY annee_scolaire, semestre")
    ...
    code_mod = modules_df.loc[modules_df["intitule"] == selected_module, "code_module"].values[0]
    ...
    q_notes = f"""
        SELECT de.nom || ' ' || de.prenom AS etudiant,
               ...
               fn.moyenne, fn.classement, fn.ecart_moyenne_classe, fn.statut_validation,
               dt.annee_scolaire
        FROM FAIT_NOTES fn
    ...
    with c1:
        if not df_moy_annee.empty:
            fig = px.line(
                ...
                markers=True, labels={"annee_scolaire": "Année scolaire", "moyenne": "Moyenne /20"},
            )
            fig.add_hline(y=10, line_dash="dash", line_color="red", annotation_text="Seuil 10")
            st.plotly_chart(fig, use_container_width=True)

    with c2:
        # Bar chart: moyenne par module (current filter year)
        q_bar = """
            SELECT dm.intitule AS module, ROUND(AVG(fn.moyenne)::numeric, 2) AS moyenne
            FROM FAIT_NOTES fn
    ...
    # Histogram of notes distribution
    if not df_notes.empty:
        fig3 = px.histogram(
    ...
    tab_top, tab_bot, tab_risk = st.tabs(["🏆 Top 5", "⚠️ Bottom 5", f"📉 En dessous de {seuil_note}/20"])
    if not df_notes.empty:
        ...
        with tab_risk:
            below = df_notes[df_notes["moyenne"] < seuil_note][display_cols]
    ...
    if not df_abs.empty:
        ...
        # Evolution des absences dans le temps
        ...
        # Slicer absences justifiées
        st.subheader("Détail absences (filtrer par justification)")
        just_filter = st.radio("Type", ["Toutes", "Justifiées (O)", "Non justifiées (N)"], horizontal=True)
    ...
    if not df_notes.empty:
        ...
        fig_risk = px.bar(
            ...
            color="niveau_risque",
            color_discrete_map={"🟢 Faible": "#28a745", "🟡 Moyen": "#ffc107", "🔴 Élevé": "#dc3545"},
        ...
        st.dataframe(
            df_risk[["etudiant", "moyenne", "taux_absence", "taux_non_rendu", "score_risque", "niveau_risque"]],
            use_container_width=True,
        )
    ...
    params_reclam: dict = {}
    if code_mod != "":
        q_reclam += " AND fr.code_module = :code_mod"
        params_reclam["code_mod"] = code_mod
    if selected_annee != "Toutes":
    ...
    if not df_reclam.empty:
        ...
        st.dataframe(
            df_reclam[["etudiant", "module", "type_reclamation", "statut", "date_depot", "date_reponse", "delai_traitement"]],
            use_container_width=True,
        )
    else:
...
# ══════════════════════════════════════════════
...
elif view == "👨‍🎓 Dashboard Étudiant":
    st.title("👨‍🎓 Tableau de Bord Étudiant")

    # ── Student selector ─────────────────────
    students_df = query(
        engine,
        "SELECT num_apogee, nom || ' ' || prenom AS nom_complet, annee_etude FROM DIM_ETUDIANT ORDER BY nom",
    )
    st.sidebar.subheader("Sélection étudiant")
    student_label = st.sidebar.selectbox("Étudiant", students_df["nom_complet"].tolist())
    student_row   = students_df[students_df["nom_complet"] == student_label].iloc[0]
    num_apogee    = int(student_row["num_apogee"])
    annee_etude   = student_row["annee_etude"]

    st.subheader(f"Bonjour, {student_label}")
    ...
    """
    df_stud_notes = query(engine, q_stud_notes, num=num_apogee)
    ...
    if not df_stud_notes.empty:
        ...
        # Détail notes par type d'évaluation
        ...
        # Écart par rapport à la classe (radar / bar)
        st.subheader("Écart par rapport à la moyenne de classe")
        fig_ecart = px.bar(
            df_stud_notes,
            x="module", y="ecart_moyenne_classe",
        ...
        # Statut validation par module
        st.subheader("Statut de validation par module")
        val_df = df_stud_notes[["module", "statut_validation", "moyenne"]].copy()
        val_df["couleur"] = val_df["statut_validation"].map({"Valide": "Validé", "Non valide": "Non validé"})
        fig_val = px.bar(
            val_df, x="module", y="moyenne",
            color="couleur",
        ...
        if not rank_df.empty:
            st.dataframe(
                rank_df.rename(columns={"module": "Module", "classement": "Rang", "annee_scolaire": "Année"}),
                use_container_width=True,
            )
    else:
    ...
    """
    df_stud_abs = query(engine, q_stud_abs, num=num_apogee)

    if not df_stud_abs.empty:
        col_a1, col_a2, col_a3 = st.columns(3)
        col_a1.metric("Total absences", int(df_stud_abs["nb_absences_total"].iloc[0]) if len(df_stud_abs) > 0 else 0)
        ...
        taux_max = df_stud_abs["taux_absence"].max()
        fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number",
            value=float(taux_max),
        ...
        st.dataframe(
            df_stud_abs[["module", "date_seance", "seance", "justifiee", "taux_absence", "seuil_depasse"]].rename(
                columns={"module": "Module", "date_seance": "Date", "seance": "Séance",
                         "justifiee": "Justifiée", "taux_absence": "Taux (%)", "seuil_depasse": "Seuil dépassé"}
            ),
            use_container_width=True,
        )
    else:
    ...
    q_stud_reclam = """
        SELECT dm.intitule AS module, dp.nom || ' ' || dp.prenom AS professeur,
               fr.type_reclamation, fr.statut,
    ...
    """
    df_stud_reclam = query(engine, q_stud_reclam, num=num_apogee)
    ...
    q_events = """
        SELECT ev.type_evenement, ev.objet, ev.date_evenement,
               ev.heure_evenement, dm.intitule AS module, ev.promotion
        FROM evenements ev
        JOIN DIM_MODULE dm ON ev.code_module = dm.code_module
        ORDER BY ev.date_evenement ASC
    """
    df_events = query(engine, q_events)

    if not df_events.empty:
        today = date.today()
        df_events["date_evenement"] = pd.to_datetime(df_events["date_evenement"])
        df_events["jours_restants"] = (df_events["date_evenement"].dt.date.apply(lambda d: (d - today).days))
        ...
        if not upcoming.empty:
            ...
            for _, ev in upcoming.iterrows():
                ...
                with st.expander(f"{color} {ev['objet']} — {ev['date_evenement'].strftime('%d/%m/%Y')} ({jj} j)"):
    ...
    if not df_stud_abs.empty and (df_stud_abs["seuil_depasse"] == "O").any():
        alertes.append("⚠️ Votre taux d'absence a dépassé le seuil réglementaire dans au moins un module.")
    if not df_stud_notes.empty:
        mods_risk = df_stud_notes[df_stud_notes["statut_validation"] == "Non valide"]["module"].tolist()
        if mods_risk:
